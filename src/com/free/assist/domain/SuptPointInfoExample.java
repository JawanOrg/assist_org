...
public class SuptPointInfoExample extends BaseExample {
	...
	public static class Criteria {
		...
		public Criteria andHomePageStatus(String homePageStatus) {
			if ("plan".equals(homePageStatus)) {
				addCriterion("exists (select 1 from t_bus_release_trends tr where tr.bill_id=name and tr.bill_status='" + Constant.S_WORK + "' and tr.repair_time>sysdate)");
			} else if ("doing".equals(homePageStatus)) {
				addCriterion("exists (select 1 from t_bus_release_trends tr where tr.bill_id=name and tr.bill_status='" + Constant.S_WORK + "' and tr.repair_time<sysdate and tr.repair_end_time>sysdate)");
			} else if ("finish".equals(homePageStatus)) {
				addCriterion("exists (select 1 from t_bus_release_trends tr where tr.bill_id=name and tr.bill_status='" + Constant.S_FINISH + "')");
			}
			return this;
		}
	}
