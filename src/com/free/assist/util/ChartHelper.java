...
public class ChartHelper {
    ...
    public static String buildXml(BusReleaseTrends project, List<SuptTask> itemList) {
        ...
        try {
            ...
            sb.append(project.getCreateTime());
            sb.append("</create_time><creator_phone>");
            sb.append(userVO.getMobilePhone());
            sb.append("</creator_phone><onclick>showDetail('").append(project.getBillId()).append("','")
                    .append(project.getBillStatus()).append("')</onclick><tips></tips></start>");
            for (SuptTask item : itemList) {
                ...
                sb.append("<node><node_id>n").append(item.getTaskId()).append("</node_id><operator>")
                        .append(userVO != null ? userVO.getUserName() : "").append("</operator><status>")
                        .append(item.getTaskStatus()).append("</status><phone>").append("</phone><create_time>")
                        .append(item.getCreateTime()).append("</create_time><end_time></end_time><remaining_time>")
                        .append("").append("</remaining_time><overtime></overtime><parent_id>n")
                        .append(item.getTaskIdParent()).append("</parent_id><onclick>showOtherInfo('")
                        .append(item.getTaskId()).append("','").append(project.getBillId())
                        .append("')</onclick><tips></tips></node>");
            }
            sb.append("</chart>");
        } catch (Exception e) {
    ...
    public static String buildPageCode(String xmlStr) {
        ...
        sb.append("<div id=\"").append(startId).append("\" class=\"window\" ondblclick=\"")
                .append(getNodeText(xmlDocument.selectSingleNode("chart/start"), "onclick"))
                .append("\"><table><tr><td class=\"td_title\">").append(getNodeText(startNode, "title"))
                .append("</td></tr><tr><td>").append(getNodeText(startNode, "user_name"))
                .append("</td></tr><tr><td><font class=\"time\">").append(getNodeText(startNode, "creator"))
                .append("(").append(getNodeText(startNode, "create_time")).append(")</font></td></tr></table></div>");
        for (Node node : nodeList) {
            sb.append("<div id=\"")
                    .append(getNodeText(node, "node_id"))
                    .append("\" class=\"window\" ondblclick=\"")
                    .append(getNodeText(node, "onclick"))
                    .append("\"><table><tr><td><font class=\"name\">")
                    .append(getNodeText(node, "operator"))
                    .append("</font>")
                    .append(getNodeText(node, "phone").length() > 0 ? ("（<font class=\"phone\">"
                            + getNodeText(node, "phone") + "</font>）") : "")
                    .append("</td></tr><tr><td class=\"td_status\">").append(getNodeText(node, "status"))
    ...
    private static String buildPosition(Node startNode, List<Node> nodeList, Document xmlDocument) {
            ...
            for (Node sonNode : nodeList) {
                if (("," + getNodeText(sonNode, "parent_id") + ",").indexOf("," + getNodeText(node, "node_id") + ",") != -1) {
                    ...
                    for (Entry<String, Node> entry : tempNodeMap.entrySet()) {
                        ...
                        if (keyNumId != null
                                && ("," + getNodeText(sonNode, "parent_id") + ",").indexOf(","
                                        + keyNumId.substring(keyNumId.indexOf("_") + 1) + ",") != -1) {
                            int currentLevel = Integer.parseInt(keyNumId.substring(0, keyNumId.indexOf("_")));
                            nodeMap.put(((currentLevel + 1) < 10 ? ("0" + (currentLevel + 1)) : (currentLevel + 1))
                                    + "_" + getNodeText(sonNode, "node_id"), sonNode);
                        }
                    }
                }
            }
        }
        int basicTop = Integer.parseInt(getNodeText(xmlDocument.selectSingleNode("chart/basicInfo"), "topSpace")), basicLeft =
                Integer.parseInt(getNodeText(xmlDocument.selectSingleNode("chart/basicInfo"), "leftSpace"));
        int level = 1;
        tempNodeMap = new HashMap<String, Node>();
        for (Entry<String, Node> entry : nodeMap.entrySet()) {
            tempNodeMap.put(entry.getKey(), entry.getValue());
            ...
            if (nodeKey != null && nodeKey.indexOf("_") != -1 && nodeKey.length() > 3) {
                ...
                if (level == 1) {
                    ...
                    for (int i = 0; i < nodePositionList.size(); i++) {
                        if (treemap.get(nodeKey) != null
                                && ("," + getNodeText(treemap.get(nodeKey), "parent_id") + ",").indexOf(","
                                        + nodePositionList.get(i)[0] + ",") != -1) {
                            nodePositionList.get(i)[3] = (Integer.parseInt(nodePositionList.get(i)[3]) + 1) + "";
                        }
                    }
                }
                nodePosition =
                        new String[] { nodeKey.substring(nodeKey.substring(0, nodeKey.indexOf("_")).length() + 1),
                                (basicTop + ((level - 1) * basicTop * 2)) + "", left + "", 0 + "" };
                        ...
                        if (treemap.get(nodeKey) != null
                            ...
                            if (Integer.parseInt(nodePositionList.get(i)[3]) == 1) {
                                for (int j = 0; j < nodePositionList.size(); j++) {
                                    if (nodeKey.substring(nodeKey.substring(0, nodeKey.indexOf("_")).length()).equals(
                                            nodePositionList.get(j)[0])) {
                                        nodePositionList.get(j)[2] = nodePositionList.get(i)[2];
                                        allNodePositionSb.append(",").append(nodePositionList.get(j)[1]).append("_")
                                                .append(nodePositionList.get(j)[2]).append(",");
                                        break;
                                    }
                                }
                                    ...
                                    if (nodeKey.substring(nodeKey.indexOf("_") + 1).equals(nodePositionList.get(j)[0])) {
                                        if (allNodePositionSb.length() > 0) {
                                            ...
                                            while (temp.indexOf("," + nodePositionList.get(j)[1] + "_") != -1) {
                                                temp =
                                                        temp.substring(temp.indexOf("," + nodePositionList.get(j)[1]
                                                                + "_")
                                                                + ("," + nodePositionList.get(j)[1] + "_").length());
                                                postitionList
                                                        .add(Integer.parseInt(temp.substring(0, temp.indexOf(","))));
                                            }
                                        }
                                        int maxPosition =
                                                postitionList.size() != 0 ? Collections.max(postitionList) : 0;
                                        if (maxPosition == 0) {
                                            nodePositionList.get(j)[2] =
                                                    (Integer.parseInt(nodePositionList.get(i)[2]) - basicLeft / 2) + "";
                                        } else {
        ...
        for (int i = 0; i < nodePositionList.size(); i++) {
            sb.append("#").append(nodePositionList.get(i)[0])
                    .append("{top: " + nodePositionList.get(i)[1] + "em; left: " + nodePositionList.get(i)[2] + "em;}");
        }
        return sb.toString();
    }
    ...
    private static String buildDragCode(Node startNode, List<Node> nodeList) {
        StringBuilder sb = new StringBuilder(256);
        sb.append("$(function(){  var ").append(getNodeText(startNode, "id")).append("_dragging = false;  var ")
                .append(getNodeText(startNode, "id")).append("_iX, ").append(getNodeText(startNode, "id"))
                .append("_iY; ");
        sb.append("$(\"#").append(getNodeText(startNode, "id")).append("\").mousedown(function(e) {")
                .append(getNodeText(startNode, "id")).append("_dragging = true;").append(getNodeText(startNode, "id"))
                .append("_iX = e.clientX - this.offsetLeft;").append(getNodeText(startNode, "id"))
                .append("_iY = e.clientY - this.offsetTop;this.setCapture && this.setCapture();return false;});");
        for (Node node : nodeList) {
            sb.append(" var ").append(getNodeText(node, "node_id")).append("_dragging = false;  var ")
                    .append(getNodeText(node, "node_id")).append("_iX, ").append(getNodeText(node, "node_id"))
                    .append("_iY; ");
            sb.append("$(\"#").append(getNodeText(node, "node_id")).append("\").mousedown(function(e) {")
                    .append(getNodeText(node, "node_id")).append("_dragging = true;")
                    .append(getNodeText(node, "node_id")).append("_iX = e.clientX - this.offsetLeft;")
                    .append(getNodeText(node, "node_id"))
                    .append("_iY = e.clientY - this.offsetTop;this.setCapture && this.setCapture();return false;});");

        }
        sb.append("document.onmousemove = function(e) {if (").append(getNodeText(startNode, "id"))
                .append("_dragging) {var e = e || window.event;var ").append(getNodeText(startNode, "id"))
                .append("_oX = e.clientX - ").append(getNodeText(startNode, "id")).append("_iX;var ")
                .append(getNodeText(startNode, "id")).append("_oY = e.clientY - ").append(getNodeText(startNode, "id"))
                .append("_iY;");
        sb.append("$(\"#").append(getNodeText(startNode, "id")).append("\").css({\"left\":")
                .append(getNodeText(startNode, "id")).append("_oX + \"px\", \"top\":")
                .append(getNodeText(startNode, "id")).append("_oY + \"px\"});return false;};");
        for (Node node : nodeList) {
            sb.append(" if (").append(getNodeText(node, "node_id"))
                    .append("_dragging) {var e = e || window.event;var ").append(getNodeText(node, "node_id"))
                    .append("_oX = e.clientX - ").append(getNodeText(node, "node_id")).append("_iX;var ")
                    .append(getNodeText(node, "node_id")).append("_oY = e.clientY - ")
                    .append(getNodeText(node, "node_id")).append("_iY;");
            sb.append("$(\"#").append(getNodeText(node, "node_id")).append("\").css({\"left\":")
                    .append(getNodeText(node, "node_id")).append("_oX + \"px\", \"top\":")
                    .append(getNodeText(node, "node_id")).append("_oY + \"px\"}); return false;};");
        }
        sb.append("};");
        sb.append("$(document).mouseup(function(e) {").append(getNodeText(startNode, "id"))
                .append("_dragging = false;e.cancelBubble = true;");
        sb.append("$(\"#").append(getNodeText(startNode, "id")).append("\")[0].releaseCapture();chart.repaint(\"")
                .append(getNodeText(startNode, "id")).append("\");");
        for (Node node : nodeList) {
            sb.append(getNodeText(node, "node_id")).append("_dragging = false;");
            sb.append("$(\"#").append(getNodeText(node, "node_id")).append("\")[0].releaseCapture();chart.repaint(\"")
                    .append(getNodeText(node, "node_id")).append("\");");
        }
        sb.append("}) })");
    ...
    public static String getNodeText(Node node, String name) {
        if (node != null && name != null && node.selectSingleNode(name) != null
                && node.selectSingleNode(name).getText() != null) {
    ...
    private static String buildConnectCode(Node startNode, List<Node> nodeList) {
        ...
        for (Node node : nodeList) {
            if (("," + getNodeText(node, "parent_id") + ",").indexOf("," + startId + ",") != -1) {
                sb.append("chart.connect({source : \"")
                        .append(startId)
                        .append("\",target : \"")
                        .append(getNodeText(node, "node_id"))
                        .append("\",connector:\"Straight\",paintStyle:connectorPaintStyle,hoverPaintStyle:connectorHoverStyle});");
            }
            for (Node sonNode : nodeList) {
                if (("," + getNodeText(sonNode, "parent_id") + ",").indexOf("," + getNodeText(node, "node_id") + ",") != -1) {
                    sb.append("chart.connect({source : \"")
                            .append(getNodeText(node, "node_id"))
                            .append("\",target : \"")
                            .append(getNodeText(sonNode, "node_id"))
                            .append("\",connector:\"Straight\",paintStyle:connectorPaintStyle,hoverPaintStyle:connectorHoverStyle});");
                }
            }
        }
