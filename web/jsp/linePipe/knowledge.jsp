...
<body bgcolor="#FFFFFF" leftmargin="0" marginwidth="0" marginheight="0"
	...
	<script>
		...
		function deleteKnowledge(contentId) {
			var result = ymPrompt.confirmInfo({message:'确定要删除此管线常识信息？',handler:doDeleteKnowledge});
			alert(contentId);
			alert(result);
			if(result=='ok'){
				alert('ddd');
		...
		function doDeleteKnowledge(contentId) {
			
			//ymPrompt.confirmInfo({message:'确定要删除此管线常识信息？',handler:handler});
		}
