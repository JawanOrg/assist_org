...
function locationCard(settings) {
    ...
    var _options = jq.fn.extend({ //合并配置参数
        ids: [],
    ...
    var pInput = jq('#' + provinceInputId);
    var cInput = jq('#' + cityInputId);
    var xInput = countyInputId ? jq('#' + countyInputId) : null;
    ...
    function createP() {
        ...
        html += '</div>';
        pdiv = jq(html);
        if (jq('#__' + provinceInputId).length == 0) {
    ...
    this.init = function () {
        ...
        jq(document).delegate('html', 'click', function (event) {
