function showDetail(projectId,status){
	var url = "/action/ProjectMessageAction.do?method=entry&projectId="+projectId+"&status="+status+"&pageType=T";
	window.location.href= url;
}
function showOtherInfo(itemId,status){
	var url = "/ganttView.jsp?projectId="+itemId+"&status="+status+"&pageType=T";
	window.location.href = url;
}
(function($) {
...
var throwError=function(message) {
    throw({name:"plugin error",message:message});
};
...
chartUtil = {
	...
	gradient : function(b, a) {
		b = chartUtil.isArray(b) ? b : [ b.x, b.y ];
		a = chartUtil.isArray(a) ? a : [ a.x, a.y ];
		return (a[1] - b[1]) / (a[0] - b[0])
	},
	normal : function(b, a) {
		return -1 / chartUtil.gradient(b, a)
	},
	...
	pointOnLine : function(a, e, b) {
		var d = chartUtil.gradient(a, e), i = chartUtil.segment(a, e), h = b > 0 ? chartUtil.segmentMultipliers[i]
				: chartUtil.inverseSegmentMultipliers[i], c = Math.atan(d), f = Math
				.abs(b * Math.sin(c))
	...
	perpendicularLineTo : function(c, d, e) {
		var b = chartUtil.gradient(c, d), f = Math.atan(-1 / b), g = e / 2
	...
	indexOf : function(a, b) {
		return chartUtil.findWithFunction(a, function(c) {
			return c == b
		})
	},
	removeWithFunction : function(c, d) {
		var b = chartUtil.findWithFunction(c, d);
		if (b > -1) {
			c.splice(b, 1)
		}
		return b != -1
	...
	addWithFunction : function(c, b, a) {
		if (chartUtil.findWithFunction(c, a) == -1) {
			c.push(b)
		}
	},
	...
	EventGenerator : function() {
		...
		this.bind = function(d, e) {
			chartUtil.addToList(c, d, e);
			return b
		};
		this.fire = function(g, h, d) {
			if (c[g]) {
				for ( var f = 0; f < c[g].length; f++) {
					if (chartUtil.findWithFunction(a, function(i) {
						return i === g
					}) != -1) {
						c[g][f](h, d)
					} else {
						try {
							...
							chartUtil.log("chart: fire failed for event " + g
									+ " : " + k)
						}
					}
				}
	...
	logEnabled : true,
	log : function() {
		if (chartUtil.logEnabled && typeof console != "undefined") {
			try {
...
(function() {
	...
	var c = function(i) {
		...
		this.updateOffsets = function(o) {
			var r = chart.CurrentLibrary, m = r.getElementObject(o), l = i
		...
		this.endpointAdded = function(m) {
			var r = chart.CurrentLibrary, u = document.body, k = i.getId(m), t = r
			...
			while (l != u) {
				...
				if (q && h[q]) {
					...
					if (f[q][k] == null) {
						var v = chart.CurrentLibrary.getOffset(m);
						f[q][k] = {
							id : k,
							offset : {
								left : v.left - n.left,
...
(function() {
	var g = chartUtil.findWithFunction, G = chartUtil.indexOf, A = chartUtil.removeWithFunction, k = chartUtil.remove, r = chartUtil.addWithFunction, i = chartUtil.addToList, l = chartUtil.isArray, z = chartUtil.isString, u = chartUtil.isObject;
	var v = null, c = function(H, I) {
	...
	}, o = chartUtil.log, F = chartUtil.group, f = chartUtil.groupEnd, E = chartUtil.time, t = chartUtil.timeEnd, p = function() {
		...
		U.hoverClass = aa.hoverClass || U._chart.Defaults.HoverClass
				|| n.Defaults.HoverClass;
		chartUtil.EventGenerator.apply(this);
		this.clone = function() {
			var ad = new Object();
		...
		this.isDetachAllowed = function(ad) {
			var ae = U._chart.checkCondition("beforeDetach", ad);
			if (L) {
				try {
					ae = L(ad)
				} catch (af) {
					o("chart: beforeDetach callback failed", af)
				}
			}
			return ae
		};
		...
		this.isDropAllowed = function(ai, af, ag, ad, ae) {
			var ah = U._chart.checkCondition("beforeDrop", {
				sourceId : ai,
				targetId : af,
				scope : ag,
				connection : ad,
			...
			if (O) {
				try {
					...
					o("chart: beforeDrop callback failed", aj)
				}
			}
			return ah
		};
		...
		this.setHover = function(ad, af, ae) {
			if (!U._chart.currentlyDragging && !U._chart.isHoverSuspended()) {
				S = ad;
		...
		var H = [], P = function(ad) {
			...
			if (U.getDefaultType) {
				...
				var af = chartUtil.merge({}, U.getDefaultType());
				for ( var ad = 0; ad < H.length; ad++) {
					af = chartUtil.merge(af, U._chart.getType(H[ad], ag))
				}
				U.applyType(af);
				if (!ae) {
					U.repaint()
		...
		U.hasType = function(ad) {
			return chartUtil.indexOf(H, ad) != -1
		};
		...
		U.toggleType = function(ag, ah) {
			...
			if (af != null) {
				for ( var ae = 0; ae < af.length; ae++) {
					var ad = chartUtil.indexOf(H, af[ae]);
					if (ad != -1) {
						H.splice(ad, 1)
					} else {
						H.push(af[ae])
		...
		var K = function(X) {
			...
			if (l(X)) {
				...
				V = new n.Overlays[S._chart.getRenderMode()][U](W);
				if (W.events) {
				...
				if (X.constructor == String) {
					V = new n.Overlays[S._chart.getRenderMode()][X]({
						component : S,
						_chart : S._chart
					})
				} else {
			...
			var T = S.defaultOverlayKeys || [], W = X.overlays, U = function(Y) {
				return S._chart.Defaults[Y] || n.Defaults[Y] || []
			};
			if (!W) {
				W = []
			}
		...
		var J = "__label", R = function(V) {
			...
			return new n.Overlays[S._chart.getRenderMode()].Label(U)
		};
		if (M.label) {
			var P = M.labelLocation || S.defaultLabelLocation || 0.5, Q = M.labelStyle
					|| S._chart.Defaults.LabelStyle || n.Defaults.LabelStyle;
	...
	var w = function(I) {
		...
		this.connectorClass = "_chart_connector";
		this.endpointClass = "_chart_endpoint";
		this.overlayClass = "_chart_overlay";
		this.Anchors = {};
		this.Connectors = {
			canvas : {},
			svg : {},
		...
		var ba = function(bC, bE, bz) {
			var bB = br.Defaults.ConnectionType
				...
				targetEndpoint : bC.endpoints[1]
					...
					: bC;
			if (bE) {
				br.fire("chartConnectionDetached", bD, bz);
		...
		var aY = function(bz) {
			return function() {
				return chartAdapter.isRenderModeAvailable(bz)
			}
		};
		...
		this.setRenderMode = function(bz) {
			Y = chartAdapter.setRenderMode(bz);
			return Y
		};
		...
		var az = function(bU) {
			...
			this.getDefaultType = function() {
				return {
					parameters : {},
					scope : null,
					detachable : bK._chart.Defaults.ConnectionsDetachable,
					rettach : bK._chart.Defaults.ReattachConnections,
					paintStyle : bK._chart.Defaults.PaintStyle
							|| n.Defaults.PaintStyle,
					connector : bK._chart.Defaults.Connector
							|| n.Defaults.Connector,
					hoverPaintStyle : bK._chart.Defaults.HoverPaintStyle
							|| n.Defaults.HoverPaintStyle,
					overlays : bK._chart.Defaults.ConnectorOverlays
		...
		var ah = function(b5) {
			...
			this.getDefaultType = function() {
				return {
					parameters : {},
					scope : null,
					maxConnections : bS._chart.Defaults.MaxConnections,
					paintStyle : bS._chart.Defaults.EndpointStyle
							|| n.Defaults.EndpointStyle,
					endpoint : bS._chart.Defaults.Endpoint
							|| n.Defaults.Endpoint,
					hoverPaintStyle : bS._chart.Defaults.EndpointHoverStyle
							|| n.Defaults.EndpointHoverStyle,
					overlays : bS._chart.Defaults.EndpointOverlays
	...
	if (typeof define === "function" && define.amd && define.amd.chart) {
	...
	var q = function(H, M, J, I, L, K) {
		return function(O) {
			O = O || {};
			var N = O.chartInstance.makeAnchor([ H, M, J, I, 0, 0 ],
					O.elementId, O.chartInstance);
			N.type = L;
			if (K) {
				K(N, O)
			}
	...
	n.Defaults.DynamicAnchors = function(H) {
		return H.chartInstance.makeAnchors([ "TopCenter", "RightMiddle",
				"BottomCenter", "LeftMiddle" ], H.elementId, H.chartInstance)
	};
	n.Anchors.AutoDefault = function(I) {
		var H = I.chartInstance.makeDynamicAnchor(n.Defaults.DynamicAnchors(I));
		H.type = "AutoDefault";
		return H
	};
	n.Anchors.Assign = q(
			...
			function(I, J) {
				...
				I.positionFinder = H.constructor == String ? J.chartInstance.AnchorPositionFinders[H]
						: H;
				I.constructorParams = J
			});
	n.Anchors.Continuous = function(H) {
		return H.chartInstance.continuousAnchorFactory.get(H)
	};
	...
	n.Anchors.Perimeter = function(H) {
		...
		var O = H.chartInstance.makeDynamicAnchor(R);
		O.type = "Perimeter";
		return O
	}
})();
(function() {
	chart.DOMElementComponent = function(d) {
		chart.chartUIComponent.apply(this, arguments);
		this.mousemove = this.dblclick = this.click = this.mousedown = this.mouseup = function(
				f) {
		}
	};
	chart.Connectors.Straight = function() {
		this.type = "Straight";
		...
		this.pointAlongPathFrom = function(t, x, w) {
			...
			return chartUtil.pointOnLine(v, u, x)
		}
	};
	chart.Connectors.Bezier = function(w) {
		var q = this;
		w = w || {};
	...
	chart.Connectors.Flowchart = function(A) {
		this.type = "Flowchart";
		A = A || {};
		var s = this, e = A.stub || A.minStubLength || 30, k = chartUtil
				.isArray(e) ? e[0] : e, p = chartUtil.isArray(e) ? e[1] : e, u = A.gap || 0, f = A.midpoint || 0.5, v = [], n = 0, h = [], r = [], w = [], t, q, z = -Infinity, x = -Infinity, B = Infinity, y = Infinity, d = A.grid, g = function(
				H, D) {
		...
		this.compute = function(aa, ao, D, U, az, O, Y, T, au, aq) {
			...
			var W = R[0] == 0 ? "y" : "x", Q = af ? "opposite"
					: G ? "orthogonal" : "perpendicular", J = chartUtil
	...
	chart.Endpoints.Dot = function(e) {
		this.type = "Dot";
		var d = this;
		e = e || {};
	...
	chart.Endpoints.Rectangle = function(e) {
		this.type = "Rectangle";
		var d = this;
		e = e || {};
	...
	var b = function(f) {
		chart.DOMElementComponent.apply(this, arguments);
		var d = this;
		var e = [];
	...
	chart.Endpoints.Image = function(i) {
		this.type = "Image";
		b.apply(this, arguments);
		...
		n.canvas.className = chart.endpointClass + k;
		if (f) {
		...
		chart.appendElement(n.canvas, i.parent);
		...
		var m = function(r, q, p) {
			if (!g) {
				...
				var o = n.anchorPoint[0] - (f / 2), s = n.anchorPoint[1]
						- (e / 2);
				chart.sizeCanvas(n.canvas, o, s, f, e)
			}
		};
	...
	chart.Endpoints.Blank = function(e) {
		var d = this;
		this.type = "Blank";
		b.apply(this, arguments);
		...
		d.canvas.className = d._chart.endpointClass;
		chart.appendElement(d.canvas, e.parent);
		this.paint = function(h, g, f) {
			chart.sizeCanvas(d.canvas, h[0], h[1], h[2], h[3])
		}
	};
	chart.Endpoints.Triangle = function(d) {
		this.type = "Triangle";
		d = d || {};
		d.width = d.width || 55;
	...
	chart.Overlays.Arrow = function(h) {
		this.type = "Arrow";
		...
		this.draw = function(m, s, w) {
			...
			if (m.pointAlongPathFrom) {
				if (chartUtil.isString(e.loc) || e.loc > 1 || e.loc < 0) {
					var v = parseInt(e.loc);
					y = m.pointAlongPathFrom(v, g * e.length / 2, true), A = m
							.pointOnPath(v, true), u = chartUtil.pointOnLine(y,
							A, e.length)
				} else {
					if (e.loc == 1) {
						y = m.pointOnPath(e.loc);
						A = m.pointAlongPathFrom(e.loc, -1);
						u = chartUtil.pointOnLine(y, A, e.length);
						if (g == -1) {
							var B = u;
							u = y;
							y = B
						...
						if (e.loc == 0) {
							...
							A = m.pointAlongPathFrom(e.loc, 1);
							y = chartUtil.pointOnLine(u, A, e.length);
							if (g == -1) {
								var B = u;
								u = y;
								y = B
							...
							y = m.pointAlongPathFrom(e.loc, g * e.length / 2),
									A = m.pointOnPath(e.loc), u = chartUtil
											.pointOnLine(y, A, e.length)
						}
					}
				}
				t = chartUtil.perpendicularLineTo(y, u, e.width);
				k = chartUtil.pointOnLine(y, u, d * e.length);
	...
	chart.Overlays.PlainArrow = function(e) {
		e = e || {};
		var d = chart.extend(e, {
			foldback : 1
		});
		chart.Overlays.Arrow.call(this, d);
		this.type = "PlainArrow"
	};
	chart.Overlays.Diamond = function(f) {
		f = f || {};
		var d = f.length || 40, e = chart.extend(f, {
			length : d / 2,
			foldback : 2
		});
		chart.Overlays.Arrow.call(this, e);
		this.type = "Diamond"
	};
	var a = function(i) {
		chart.DOMElementComponent.apply(this, arguments);
		...
		var h = function() {
			...
			l = chart.CurrentLibrary.getDOMElement(l);
			l.style.position = "absolute";
			var m = i._chart.overlayClass + " "
					+ (d.cssClass ? d.cssClass : i.cssClass ? i.cssClass : "");
			l.className = m;
			chart.appendElement(l, i.component.parent);
			i._chart.getId(l);
		...
		this.getDimensions = function() {
			return chart.CurrentLibrary.getSize(chart.CurrentLibrary
		...
		this.cleanup = function() {
			if (l != null) {
				chart.CurrentLibrary.removeElement(l)
			}
		};
		...
		this.draw = function(n, o, p) {
			...
			if (t != null && t.length == 2) {
				...
				if (n.pointOnPath) {
					var r = d.loc, s = false;
					if (chartUtil.isString(d.loc) || d.loc < 0 || d.loc > 1) {
						r = parseInt(d.loc);
						s = true
					}
					q = n.pointOnPath(r, s)
	...
	chart.Overlays.Custom = function(d) {
		this.type = "Custom";
		a.apply(this, arguments)
	};
	chart.Overlays.Label = function(h) {
		var d = this;
		this.labelStyle = h.labelStyle || chart.Defaults.LabelStyle;
		this.cssClass = this.labelStyle != null ? this.labelStyle.cssClass
				: null;
		h.create = function() {
			return document.createElement("div")
		};
		chart.Overlays.Custom.apply(this, arguments);
		this.type = "Label";
...
(function() {
	...
	chart.Connectors.StateMachine = function(l) {
	...
	chart.Connectors.canvas.StateMachine = function(f) {
		f = f || {};
		var d = this, g = f.drawGuideline || true, e = f.avoidSelector;
		chart.Connectors.StateMachine.apply(this, arguments);
		chart.CanvasConnector.apply(this, arguments);
	...
	chart.Connectors.svg.StateMachine = function() {
		var d = this;
		chart.Connectors.StateMachine.apply(this, arguments);
		chart.SvgConnector.apply(this, arguments);
	...
	chart.Connectors.vml.StateMachine = function() {
		chart.Connectors.StateMachine.apply(this, arguments);
		chart.VmlConnector.apply(this, arguments);
		var d = chart.vml.convertValue;
...
(function() {
	...
	if (document.createStyleSheet && document.namespaces) {
		var m = [ ".chart_vml", "chart\\:textbox", "chart\\:oval",
				"chart\\:rect", "chart\\:stroke", "chart\\:shape",
				"chart\\:group" ], g = "behavior:url(#default#VML);position:absolute;";
		c = document.createStyleSheet();
		for ( var r = 0; r < m.length; r++) {
			c.addRule(m[r], g)
		}
		document.namespaces.add("chart", "urn:schemas-microsoft-com:vml")
	}
	chart.vml = {};
	var t = 1000, s = {}, a = function(u, i) {
		var w = chart.getId(u), v = s[w];
		if (!v) {
			v = f("group", [ 0, 0, t, t ], {
				"class" : i
			});
		...
		z = z || {};
		var x = document.createElement("chart:" + u);
		if (v) {
			i.appendElement(x, w)
		} else {
			chart.CurrentLibrary.appendElement(x, w)
		}
		x.className = (z["class"] ? z["class"] + " " : "") + "chart_vml";
		k(x, y);
		e(x, z);
		return x
	}, k = function(u, i, v) {
	...
	}, p = chart.vml.convertValue = function(i) {
		...
		if (u.strokeStyle) {
			x.stroked = "true";
			var D = chartUtil.convertStyle(u.strokeStyle, true);
			x.strokecolor = D;
			b(x, D, "stroke", B);
		...
		if (u.fillStyle) {
			x.filled = "true";
			var v = chartUtil.convertStyle(u.fillStyle, true);
			x.fillcolor = v;
			b(x, v, "fill", B)
		} else {
			x.filled = "false"
	...
	}, d = chart.VmlConnector = function(v) {
		var i = this;
		i.strokeNode = null;
		i.canvas = null;
		n.apply(this, arguments);
		var u = i._chart.connectorClass
			...
			if (x != null) {
				...
				if (x.outlineColor) {
					var C = x.outlineWidth || 1, D = x.lineWidth + (2 * C), B = {
						strokeStyle : chartUtil.convertStyle(x.outlineColor),
						lineWidth : D
					};
					for ( var w in h) {
						B[w] = x[w]
					...
					if (i.bgCanvas == null) {
						...
						i.bgCanvas = f("shape", A, y, v.parent, i._chart, true);
				...
				if (i.canvas == null) {
					...
					i.canvas = f("shape", A, y, v.parent, i._chart, true);
		...
		var w = v._chart.endpointClass + (y.cssClass ? (" " + y.cssClass) : "");
		y._chart.appendElement(v.canvas, y.parent);
		if (v.tooltip) {
			v.canvas.setAttribute("label", v.tooltip)
		}
		this.paint = function(C, A, z) {
			var B = {};
			chart.sizeCanvas(v.canvas, C[0], C[1], C[2], C[3]);
			if (i == null) {
				B["class"] = w;
				i = v.getVml([ 0, 0, C[2], C[3] ], B, z, v.canvas, v._chart);
	...
	chart.Connectors.vml.Bezier = function() {
		chart.Connectors.Bezier.apply(this, arguments);
	...
	chart.Connectors.vml.Straight = function() {
		chart.Connectors.Straight.apply(this, arguments);
		d.apply(this, arguments);
		this.getPath = function(i) {
	...
	chart.Connectors.vml.Flowchart = function() {
		chart.Connectors.Flowchart.apply(this, arguments);
		d.apply(this, arguments);
	...
	chart.Endpoints.vml.Dot = function() {
		chart.Endpoints.Dot.apply(this, arguments);
		l.apply(this, arguments);
		this.getVml = function(w, x, u, v, i) {
			return f("oval", w, x, v, i)
		}
	};
	chart.Endpoints.vml.Rectangle = function() {
		chart.Endpoints.Rectangle.apply(this, arguments);
		l.apply(this, arguments);
		this.getVml = function(w, x, u, v, i) {
			return f("rect", w, x, v, i)
		}
	};
	chart.Endpoints.vml.Image = chart.Endpoints.Image;
	chart.Endpoints.vml.Blank = chart.Endpoints.Blank;
	chart.Overlays.vml.Label = chart.Overlays.Label;
	chart.Overlays.vml.Custom = chart.Overlays.Custom;
	var o = function(x, v) {
		x.apply(this, v);
		n.apply(this, v);
		var u = this, w = null;
		...
		this.paint = function(B, G, F, H, L, K) {
			...
			if (H) {
				z.stroked = "true";
				z.strokecolor = chartUtil.convertStyle(H, true)
			}
			if (F) {
				z.strokeweight = F + "px"
			}
			...
			D[3] = K[3];
			if (u.canvas == null) {
				u.canvas = f("shape", D, z, B.canvas.parentNode, B._chart, true);
		...
		this.cleanup = function() {
			if (u.canvas != null) {
				chart.CurrentLibrary.removeElement(u.canvas)
			}
		}
	};
	chart.Overlays.vml.Arrow = function() {
		o.apply(this, [ chart.Overlays.Arrow, arguments ])
	};
	chart.Overlays.vml.PlainArrow = function() {
		o.apply(this, [ chart.Overlays.PlainArrow, arguments ])
	};
	chart.Overlays.vml.Diamond = function() {
		o.apply(this, [ chart.Overlays.Diamond, arguments ])
	}
})();
(function() {
	var l = {
	...
	}, w = "stroke-dasharray", A = "dashstyle", e = "linearGradient", b = "radialGradient", c = "fill", a = "stop", z = "stroke", q = "stroke-width", h = "style", m = "none", t = "chart_gradient_", o = "lineWidth", C = {
		...
		for ( var K = 0; K < F.gradient.stops.length; K++) {
			...
			var M = chartUtil.convertStyle(F.gradient.stops[H][1], true);
			var O = f(a, {
		...
		G.setAttribute(c, E.fillStyle ? chartUtil.convertStyle(E.fillStyle,
				true) : m);
		G.setAttribute(z, E.strokeStyle ? chartUtil.convertStyle(E.strokeStyle,
				true) : m);
		if (E.lineWidth) {
	...
	var s = function(H) {
		...
		chart.chartUIComponent.apply(this, H.originalArgs);
		...
		if (H.useDivWrapper) {
			D.canvas = document.createElement("div");
			D.canvas.style.position = "absolute";
			chart.sizeCanvas(D.canvas, 0, 0, 1, 1);
		...
		H._chart.appendElement(D.canvas, H.originalArgs[0]["parent"]);
		if (H.useDivWrapper) {
		...
		this.paint = function(N, L, K) {
			if (L != null) {
				var J = N[0], O = N[1];
				if (H.useDivWrapper) {
					chart.sizeCanvas(D.canvas, N[0], N[1], N[2], N[3]);
					J = 0, O = 0
				}
				var M = n([ J, O, N[2], N[3] ]);
				if (D.getZIndex()) {
	...
	var d = chart.SvgConnector = function(E) {
		var D = this;
		s.apply(this, [ {
			cssClass : E._chart.connectorClass,
		...
		this._paint = function(L, H) {
			...
			if (H.outlineColor) {
				var I = H.outlineWidth || 1, G = H.lineWidth + (2 * I), J = chart.CurrentLibrary
						.extend({}, H);
				J.strokeStyle = chartUtil.convertStyle(H.outlineColor);
				J.lineWidth = G;
				if (D.bgPath == null) {
					D.bgPath = f("path", F);
	...
	chart.Connectors.svg.Bezier = function(D) {
		chart.Connectors.Bezier.apply(this, arguments);
		d.apply(this, arguments);
	...
	chart.Connectors.svg.Straight = function(D) {
		chart.Connectors.Straight.apply(this, arguments);
		d.apply(this, arguments);
		this.getPath = function(E) {
	...
	chart.Connectors.svg.Flowchart = function() {
		var D = this;
		chart.Connectors.Flowchart.apply(this, arguments);
		d.apply(this, arguments);
		this.getPath = function(E) {
	...
	var y = window.SvgEndpoint = function(E) {
		var D = this;
		s.apply(this, [ {
			cssClass : E._chart.endpointClass,
			originalArgs : arguments,
		...
		this._paint = function(H, G) {
			var F = chart.extend({}, G);
			if (F.outlineColor) {
				F.strokeWidth = F.outlineWidth;
				F.strokeStyle = chartUtil.convertStyle(F.outlineColor, true)
			}
			if (D.node == null) {
	...
	chart.Endpoints.svg.Dot = function() {
		chart.Endpoints.Dot.apply(this, arguments);
	...
	chart.Endpoints.svg.Rectangle = function() {
		chart.Endpoints.Rectangle.apply(this, arguments);
		y.apply(this, arguments);
		this.makeNode = function(E, D) {
			return f("rect", {
	...
	chart.Endpoints.svg.Image = chart.Endpoints.Image;
	chart.Endpoints.svg.Blank = chart.Endpoints.Blank;
	chart.Overlays.svg.Label = chart.Overlays.Label;
	chart.Overlays.svg.Custom = chart.Overlays.Custom;
	var p = function(H, F) {
		H.apply(this, F);
		chart.chartUIComponent.apply(this, F);
		this.isAppendedAtTopLevel = false;
		var D = this, G = null;
		...
		this.cleanup = function() {
			if (G != null) {
				chart.CurrentLibrary.removeElement(G)
			}
		}
	};
	chart.Overlays.svg.Arrow = function() {
		p.apply(this, [ chart.Overlays.Arrow, arguments ])
	};
	chart.Overlays.svg.PlainArrow = function() {
		p.apply(this, [ chart.Overlays.PlainArrow, arguments ])
	};
	chart.Overlays.svg.Diamond = function() {
		p.apply(this, [ chart.Overlays.Diamond, arguments ])
	};
	chart.Overlays.svg.GuideLines = function() {
		var I = null, D = this, H = null, G, F;
		chart.Overlays.GuideLines.apply(this, arguments);
...
(function() {
	var d = null, i = function(p, o) {
		return chart.CurrentLibrary.hasClass(a(p), o)
	}, a = function(o) {
		return chart.CurrentLibrary.getElementObject(o)
	}, m = function(o) {
		return chart.CurrentLibrary.getOffset(a(o))
	}, n = function(o) {
		return chart.CurrentLibrary.getPageXY(o)
	}, f = function(o) {
		return chart.CurrentLibrary.getClientXY(o)
	};
	var k = function() {
		var q = this;
		q.overlayPlacements = [];
		chart.chartUIComponent.apply(this, arguments);
		chartUtil.EventGenerator.apply(this, arguments);
		...
		this.mousemove = function(x) {
			var z = n(x), w = f(x), v = document.elementFromPoint(w[0], w[1]), y = r(
					v, "_chart_overlay");
			var u = d == null
					&& (r(v, "_chart_endpoint") || r(v, "_chart_connector"));
			if (!p && u && q._over(x)) {
				p = true;
	...
	var c = function(p) {
		...
		p._chart.appendElement(o, p.parent);
		o.style.position = "absolute";
		if (p["class"]) {
			o.className = p["class"]
		}
		p._chart.getId(o, p.uuid);
		if (p.tooltip) {
	...
	var h = chart.CanvasConnector = function(r) {
		l.apply(this, arguments);
		var o = function(v, t) {
			p.ctx.save();
			chart.extend(p.ctx, t);
			if (t.gradient) {
		...
		var p = this, q = p._chart.connectorClass + " " + (r.cssClass || "");
		p.canvas = c({
			"class" : q,
			_chart : p._chart,
			parent : r.parent,
		...
		p.paint = function(w, t) {
			if (t != null) {
				chart.sizeCanvas(p.canvas, w[0], w[1], w[2], w[3]);
				if (p.getZIndex()) {
	...
	var b = function(r) {
		var p = this;
		l.apply(this, arguments);
		var q = p._chart.endpointClass + " " + (r.cssClass || ""), o = {
			"class" : q,
			_chart : p._chart,
			parent : r.parent,
			tooltip : p.tooltip
		...
		this.paint = function(x, u, s) {
			chart.sizeCanvas(p.canvas, x[0], x[1], x[2], x[3]);
	...
	chart.Endpoints.canvas.Dot = function(r) {
		chart.Endpoints.Dot.apply(this, arguments);
		b.apply(this, arguments);
	...
	chart.Endpoints.canvas.Rectangle = function(p) {
		var o = this;
		chart.Endpoints.Rectangle.apply(this, arguments);
		b.apply(this, arguments);
	...
	chart.Endpoints.canvas.Triangle = function(p) {
		var o = this;
		chart.Endpoints.Triangle.apply(this, arguments);
		b.apply(this, arguments);
	...
	chart.Endpoints.canvas.Image = chart.Endpoints.Image;
	chart.Endpoints.canvas.Blank = chart.Endpoints.Blank;
	chart.Connectors.canvas.Bezier = function() {
		var o = this;
		chart.Connectors.Bezier.apply(this, arguments);
	...
	chart.Connectors.canvas.Straight = function() {
		...
		this._paint = function(r, t) {
			...
			if (t.dashstyle && t.dashstyle.split(" ").length == 2) {
				...
				var C = [ v[0] * t.lineWidth, v[1] * t.lineWidth ], y = (r[6] - r[4])
						/ (r[7] - r[5]), G = chartUtil.segment([ r[4], r[5] ],
						[ r[6], r[7] ]), x = o[G], u = Math.atan(y), z = Math
						.sqrt(Math.pow(r[6] - r[4], 2)
	...
	chart.Connectors.canvas.Flowchart = function() {
		var o = this;
		chart.Connectors.Flowchart.apply(this, arguments);
		h.apply(this, arguments);
	...
	chart.Overlays.canvas.Label = chart.Overlays.Label;
	chart.Overlays.canvas.Custom = chart.Overlays.Custom;
	var g = function() {
		chart.chartUIComponent.apply(this, arguments)
	};
	var e = function(p, o) {
	...
	chart.Overlays.canvas.Arrow = function() {
		e.apply(this, [ chart.Overlays.Arrow, arguments ])
	};
	chart.Overlays.canvas.PlainArrow = function() {
		e.apply(this, [ chart.Overlays.PlainArrow, arguments ])
	};
	chart.Overlays.canvas.Diamond = function() {
		e.apply(this, [ chart.Overlays.Diamond, arguments ])
	}
})();
(function(a) {
	chart.CurrentLibrary = {
		addClass : function(c, b) {
			c = chart.CurrentLibrary.getElementObject(c);
			try {
				if (c[0].className.constructor == SVGAnimatedString) {
					chartUtil.svg.addClass(c[0], b)
				}
			} catch (d) {
			}
			c.addClass(b)
		...
		appendElement : function(c, b) {
			chart.CurrentLibrary.getElementObject(b).append(c)
		},
		ajax : function(b) {
			b = b || {};
			b.type = b.type || "get";
		...
		bind : function(b, c, d) {
			b = chart.CurrentLibrary.getElementObject(b);
			b.bind(c, d)
		},
		dragEvents : {
			start : "start",
		...
		getParent : function(b) {
			return chart.CurrentLibrary.getElementObject(b).parent()
		},
		...
		getTagName : function(b) {
			var c = chart.CurrentLibrary.getElementObject(b);
		...
		initDraggable : function(c, b, d) {
			...
			b.helper = null;
			if (d) {
				b.scope = b.scope || chart.Defaults.Scope
			}
			c.draggable(b)
		},
		...
		removeClass : function(c, b) {
			...
			try {
				if (c[0].className.constructor == SVGAnimatedString) {
					chartUtil.svg.removeClass(c[0], b)
				}
			} catch (d) {
			}
			c.removeClass(b)
		...
		removeElement : function(b, c) {
			chart.CurrentLibrary.getElementObject(b).remove()
		},
		...
		setOffset : function(b, c) {
			chart.CurrentLibrary.getElementObject(b).offset(c)
		},
		trigger : function(d, e, b) {
			var c = jQuery._data(chart.CurrentLibrary.getElementObject(d)[0],
					"handle");
			c(b)
		},
